...
@Mod.EventBusSubscriber(modid = YetAnotherFishMod.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class YAFMForgeEvents {
    ...
    public static void onLootLoad(LootTableLoadEvent event) {
        ...
        if (name.equals(BuiltInLootTables.FISHING_FISH)) {
            if (pool!=null){
                addEntry(pool, getInjectEntry(new ResourceLocation(YetAnotherFishMod.MOD_ID, "gameplay/fishing/junk"),
                        25, 1));

                addEntry(pool, getInjectEntry(new ResourceLocation(YetAnotherFishMod.MOD_ID, "gameplay/fishing/treasure"),
                        20, 1));
            }
        }
    }
